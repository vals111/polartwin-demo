...
export const ALL_DOMAIN_NODES: Record<string, DomainCardNode> = {
  ...
  energy_fuel: {
    id: 'energy_fuel',
    name: 'Energy, Power & Fuel Depot',
    shortDesc: 'Central Microgrid Generation, Solar PV Arrays, Battery Reserve & Arctic Diesel Depot',
    tier: 'TIER 2 • ENERGY & FUEL RESERVE',
    tierNumber: 2,
    route: 'energy',
    icon: Zap,
    color: '#f59e0b',
  ...
  personnel: {
    ...
    shortDesc: 'Crew Headcount, Circadian Diurnal Demand & Life Safety',
...
export function extractLiveDomainData(targetStationId: string, snap: any) {
  ...
  return {
    ...
    energy_fuel: {
      ...
      status: isM ? 'Gen #1 & Depot Active' : 'Triple CHP & ISO Farm Sync',
      ...
      resupplyEtaDays: fl?.resupply_eta_days ?? (isM ? 88 : 102),
      trend: isM ? [62, 64, 65, 68, 70, 68] : [74, 76, 78, 82, 84, 82],
      architecture: isM ? 'Integrated Dual Diesel Gensets, 22 kW Solar PV & 6-Tank AGO Bulk Depot' : 'Combined Heat & Power (CHP), 35 kW Solar PV & ISO Fuel Farm',
    },
    water: {
